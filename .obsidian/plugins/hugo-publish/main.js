...
var HugoPublishSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("export blog tag").setDesc("Export ${blog tag} to hugo md file's header").addToggle((toggle) => toggle.setValue(this.plugin.settings.export_blog_tag).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("blog dir").setDesc("All blog copy to this dir, relative path to site, note that content will be deleted first when syncing").addText((text4) => text4.setPlaceholder("blog/dir").setValue(this.plugin.settings.blog_dir).onChange(async (value) => {
      this.plugin.settings.blog_dir = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("static dir").setDesc("All static(like images) copy to static/${static dir}, like static/ob, relative path to site/static. Can be empty, note that content will be deleted first when syncing").addText((text4) => text4.setPlaceholder("./").setValue(this.plugin.settings.static_dir).onChange(async (value) => {
